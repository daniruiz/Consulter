...
public class PaginaRanking extends Pagina{
    ...
    protected void cargarDatos(){
        try {
            String selectRanking = 	""+            
                "SELECT  COUNT(CP.IDCITA) AS TOTAL_CITAS, " +
                "		M.IDMEDICO, M.NOMBRE || ' ' || M.APELLIDO AS MEDICO, " +
                "		ME.IDESPECIALIDAD, " +
                "		E.NOMBRE AS ESPECIALIDAD " +
                "FROM CITAS_PACIENTES CP " +
                "	INNER JOIN PERSONAL_MEDICO M  ON CP.IDMEDICO = M.IDMEDICO " +
                "	INNER JOIN MEDICO_ESPECIALIDAD ME  ON M.IDMEDICO = ME.IDMEDICO " +
                "	INNER JOIN ESPECIALIDAD E  ON ME.IDESPECIALIDAD = E.ID AND ME.ACTUAL = 1 " +
                "GROUP BY  M.IDMEDICO, M.NOMBRE, M.APELLIDO, ME.IDESPECIALIDAD, E.NOMBRE  ORDER BY ESPECIALIDAD ASC, TOTAL_CITAS  DESC";
